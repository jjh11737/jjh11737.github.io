...
以GNU为例：编译包含<strong>预处理</strong>（Pre-Processor即<strong>cpp</strong>，看预处理指令，读取头文件的内容并直接插入源程序，得到另外一个c程序，再处理得到<strong>.i</strong>文本文件）、<strong>编译</strong>（Compiler即<strong>cc1</strong>，把C的语言翻译为汇编语言<strong>.s</strong>文件）、<strong>汇编</strong>（Assembler即<strong>as</strong>把.s翻译为机器码变成可重定位的<strong>.o</strong>文件）、<strong>链接</strong>（Linker用<strong>ld</strong>，把其他相关的.o比如我们调用了printf函数，那么printf.o[一个提前编译好的目标文件，来自标准库比如libc]连接到我们输出的.o文件，正是因为要对输出的.o文件进行调整，所以才叫可重定位文件）
...
> <strong><u>上下文</u></strong>本身就是记录进程的状态信息（快照），主要包含这个进程接下来执行所必须的信息：
>
...
**虚拟地址空间**，其实是由多级存储一起构成的，不仅仅是DRAM主存，而它通过页表把虚拟的地址映射到实际的地址当中，而且往往我们可以通过复用用更少的**物理内存（RSS---Real Set Size）**来让进程获得更大的**虚拟内存（VSS--Virtual Set Size）**，对于不同的进程之间往往逻辑上这些虚拟内存本身是隔离的。
...
无符号的我们没啥好说的，主要是有符号的补码，特别地<strong><u>有符号和无符号最后截断得到的结果向量是一致的</u></strong>
...
- <strong><u>规格化(Normalized)</u></strong>：阶码不全为0或1
  ...
  阶码e：1～255表示的不是真的阶数，真正阶数<strong><u>E = e - Bias</u></strong> 需要减去一个偏置量（取决于阶码位数[2^(el-1)]，float为[2^(8-1)-1]=127，double为[2^(11-1)-1]=1023），因此我们可知float范围就是[-126,127]
  ...
  小数frac：就像前面说的，最高位永远是1,所以我们就省略这一位，<strong><u>M = f + 1</u></strong>
- <strong><u>非规格化(De-normalized)</u></strong>：阶码全为0
  ...
  - 表示了0，e与f全为0。<strong><u>s=0为+0（正零），s=1为-0（负零）</u></strong>
  - 表示非常接近0的数，注意<strong><u>这里并没有一个被省略的1</u></strong>(显然)，以及<strong><u>E = 1-Bias</u></strong>，
  - 非常巧的是，这里非规格化的f如果向上溢出刚好就直接到e，刚好就是正常溢出的值
- <strong><u>特殊值(Special)</u></strong>：阶码全为1，分为2类，一种表示无穷大/无穷小，另一种表示不是一个数NaN
  ...
  - Infinity：阶码全1，小数字段全为0，无穷大，<strong>s=0为正无穷大，s=1为负无穷大</strong>
...
而实际上<strong><u>指令</u></strong>一般都是由<strong>操作码（operation code）</strong>和<strong>操作数（operation num）</strong>组成的。操作数有3种：<strong><u>立即数/寄存器/内存引用</u></strong>
...
- **movq操作数为立即数时，它一定是32位2的补码形式，之后再进行符号扩展到64位**
  ...
  注意，只有<strong><u>movl会把目标寄存器的高32位清空</u></strong>（x86-64规定，为了兼容32位操作）

  ------

- 源操作数位数 < 目标操作数位数，需要用<strong><u>零扩展</u></strong>或<strong><u>符号扩展</u></strong>
...
- <strong><u>大转小</u></strong>不用movz/movs，直接mov即可
- 以及一般的<strong>mov指令必须要求前后寄存器长度是一致的</strong>！
- `movl` / `movzbl`都是会<strong>自动高位清零</strong>的（特别注意byte需要先补0）
- <strong>CPU同时拉伸数据长度和写入内存是不可能完成的</strong>！可以善用MOVL自动清高位性质
...
使用这种方法可以进行加法和有限形式的乘法，特别适合这种简单的式子，可以大大提高效率<strong>e.g.</strong> 
...
而主要问题是我们乘法如果是两个64位，为了不丢失精度，结果应该是<strong><u>128位</u></strong>的。
因此`mulq`/`imulq`采用了`%rax`作为第64位，`%rdx`作为高64位，二者构成寄存器对。此时这两个指令只有一个操作数`%reg`，等价于: <strong><u>（这两条指令一个数字必定在%rax，另一个作为S给出）</u></strong>
`%rdx(高)`|`rax(低)` <- `%reg` * `%rax`
<strong>e.g.</strong>
...
功能：一条`SET`指令的目的操作数是一个<strong><u>低位单字节</u></strong>寄存器元素 / 一字节内存，指令会读取状态寄存器把这个这个字节设置为0 / 1
...
这是因为<strong><u>条件跳转指令需要处理器去预测各分支的结果，但是条件传送不需要</u></strong>，因为这涉及流水线，CPU不会等一条指令跑完再下一条，而是把指令分为多个阶段提前处理，因此碰到跳转时CPU不清楚是要执行跳转还是非跳转的代码，因此会猜测一个方向提前处理，但如果猜错这个方向的提前处理就必须放弃，需要整个回滚。而<strong><u>条件传送就是把两个方向需要的计算都算好根据条件搬运数据，流水线始终是满载的，不会出问题</u></strong>
...
这是 GCC 在<strong>不开启优化（`-Og`）</strong>时最喜欢的逻辑。
...
一旦有一个过程需要的空间超过寄存器的大小，就需要在栈(stack)上面分配空间，称为**栈帧（stack fram）**前面说过栈是从高地址向下增长的，栈指针`%rsp`指向栈顶元素。
比如P调用Q时，会把返回地址压入栈中，指明当Q返回的时候要从哪里继续执行，他是P的一部分<img src="https://raw.githubusercontent.com/jjh11737/jjh-blog-images/master/imgs/image-20260213171705960.png" alt="image-20260213171705960" style="zoom:67%;" align = "left" />.
...
一旦参数超过6个就会需要通过栈，<strong><u>而在栈当中每个参数的地址就需要按8字节对齐，无论其长度多少</u></strong>
先把1~6号传递到寄存器，然后把剩下的放到寄存器里面（参数7在栈顶），这部分就是所谓的<strong><u>“参数构造区”</u></strong>，它属于调用者A的栈帧，但是也可以被所调用者B使用（只要movq 8(%rsp), %rax就能访问参数7，因为他就在这个位置)，等B完成后返回此时A会把栈顶指针加上长度把这部分内容丢弃
...
<strong><u>x86-64数据永远是小端的</u></strong>，也就是低位数据存储在更低的地址，而且<strong><u>数据的地址就是最低那位的地址</u></strong>，读取数据时是<strong><u>从低位往高位读取</u></strong>
...
<strong>对齐原则</strong>就是<strong>任何K字节大小的基本对象</strong>的<strong><u>地址必须是K的倍数</u></strong>
...
同时缓冲区溢出更大的问题是他会让程序执行它本来不想执行的函数，通常我们输入一个字符串，它包含一些可执行程序的**字节编码(exploit code)**，另外还有一些字节会用一个指向攻击代码的指针覆盖返回地址，那么ret时就会跳转过去。因此任何对外的接口都必须做好保护。
...
在linux里面这已经成为标准行为，它属于<strong><u>地址空间布局随机化（ASLR）</u></strong>，它使得运行时程序的不同部分（代码/库代码/栈/全局变量/堆等）会被加载到内存的不同区域。
...
显然，如果栈帧本身是动态长度的，我们就不可以直接通过rsp找到返回地址以及各种<u>局部变量（这是主因，因为生成汇编代码时<strong>我们不知道局部变量相对于rsp的偏移，也就无法生成高效的代码</strong>）</u>，为了管理变长的栈帧，x86-64使用<strong><code>%rbp</code>寄存器</strong>作为<font color="red"><strong><u>帧指针（frame/base pointer）</u></strong></font>，这也是为什么叫bp。
它的作用就是作为一个固定的地址指针，时刻指向当前栈帧的底部，之后rsp无论怎么动我们都可<strong><u>直接通过rbp找回局部变量的位置</u></strong>（他们一般都是挨着rbp的，而变长数组会靠近栈顶指针rsp）。

那么再来解释一下上面这个代码的几个地方，先压栈保存rbp，然后把rbp变成当前栈顶指针rsp的位置作为之后的栈底（返回地址则是rbp+8处），之后先分配给i空间，我们知道<strong><u>调用 call 前 %rsp 必须是 16 的倍数</u></strong>，之前push q导致现在地址是16n+8，而int是8,现在应该是16的倍数，但是call本身又会+8,为了保证之后调用其他函数不出问题我们这里才先额外分配个8字节。之后先把rax变成22 + 8 * n（<strong><u>这里的22= 23 = 8 + 15 - 1，8给的是金丝雀，15是为了刚好向上取整，最关键的是这个1,因为事实上一个8的倍数加上22或23再去掉低16位是等价的！！</u></strong>），再通过地址掩码（16 = 0x0001 0000,-16就是取反加一0x1111 0000，<strong>清空了低4位</strong>)，这就相当于向上取整保证了结果是16的倍数，这么做还是保证rsp的16倍问题。
...
3. 把整数转化为浮点数：
   ...
   4. 最后是两种浮点格式之间的转换
      ...
      （但是GCC这么做好像没啥意义？
      意义其实是为了<strong><u>避免伪依赖</u></strong>，因为`vcvtss2sd`只操作寄存器的低位，很容易让CPU认为高位是需要保留的，换句话说它必须做完所有对此寄存器的操作后才运行这条指令执行，而这么交叉一下（就很像寄存器先自己异或一下自己一样，<strong><u>告诉CPU这个寄存器过去的值对我无所谓</u></strong>）
...
AVX2的简单的标量浮点运算指令：（注意<strong><u>S1寄存器和内存都可以，S2/D必须是XMM寄存器）</u></strong>![1](https://raw.githubusercontent.com/jjh11737/jjh-blog-images/master/imgs/image-20260303103245077.png)
比如一个简单的函数：
...
而<strong><u>对于浮点数则是只要S1/S2有一个是NaN就触发奇偶`pf`</u></strong>
具体规则如下了：
...
比如：
![img1](https://raw.githubusercontent.com/jjh11737/jjh-blog-images/master/imgs/image-20260303110007319.png)
...
我们可以优化这个问题，这个问题本质是因为使用的是这个dest，它本身是指向内存的，但是如果你<strong>使用一个临时变量acc来积累这个过程，那寄存器就可以处理了</strong>（换句话说如果你不需要一个全局的值来存储它，<strong>用局部变量完全更快</strong>）
...
由于指令级并行以及分支结构的流水线冒险，性能大大提高。但是如果指令本身有严格的先后依赖关系，指令本身会产生<strong>延迟界限</strong>：即<strong><u>下一条指令开始之前这条指令必须先结束</u></strong>
...
DRAM芯片中的单元被分为$d$个<strong>超单元($supercell$)</strong>，每个超单元由$w$个DRAM单元组成，超单元被组织成一个$r$ 行$c$列的长方形阵列($rc=d$)，用$(i,j)$表示地址

每个DRAM芯片被连接到被称为<strong>内存控制器</strong>的电路，它可以一次传送$w$位到每个DRAM芯片或一次从每个DRAM芯片传出$w$ 位，为了读取$(i,j)$的，内存控制器发送行地址$i$称为<strong>RAS（Row Access Strobe行访问脉冲）</strong>请求，列地址$j$称为<strong>CAS（Column Access Strobe列访问脉冲）</strong>请求，RAS和CAS请求共享相同的DRAM地址引脚
...
原本面密度小的时候，每个磁道被分为数码相同的扇区，扇区数目取决于最靠内侧磁道所能记录的扇区数，因此越往外间隙越大；而随着面密度增大，间隙已经不可接受了，因此引入了<strong>多区记录(mutiple zone recording)</strong>的技术
而一个磁盘容量的计算公式如下：（`#*`表示变量）
$$
...
磁盘用读写头(read/write head)来读写存储在磁性表面的位，而读写头连接到一个<strong>传动臂(actuator arm)</strong>的一端，通过沿着半径轴前后移动这个传动臂，驱动器可以把读写头定位在盘面上的任意磁道，这个机械运动叫作<strong>寻道(seek)</strong>。有多个盘面的磁盘自然有分别的读写头，而读写头都是共柱面的，垂直放置统一行动
![image-20260716082105583](https://raw.githubusercontent.com/jjh11737/jjh-blog-images/master/imgs/image-20260716082105583.png)
读写头隔着一层薄薄的气垫(0.1 micron)上移动，速度达到80km/h，显然只要有一点的微粒都会如同巨石一般，碰到读写头就会发生<strong>读写头冲撞(head crash)</strong>，因此磁盘必须密封包装
磁盘以扇区大小的块来读写数据，对扇区的<strong>访问时间(access time)</strong>由3个主要部分构成：
...
为了向操作系统隐藏实际磁盘构造的复杂性，往往把扇区简化为逻辑块的序列，内部封装有一个小的硬件/固件设备，称为<strong>磁盘控制器</strong>（维护实际与逻辑块的映射关系）
操作系统要执行一个IO操作时，会发送一个命令到磁盘控制器，让它读写某个逻辑块号，而控制器的固件执行一个表查找，把<strong>逻辑块号</strong>翻译为<strong>（盘面，磁道，扇区）</strong>的三元组，它唯一地标识了对应的物理扇区。而控制器再解释它进行读写
...
IO设备都是通过<strong>IO总线</strong>，比如intel的<strong>外围设备互联总线（PCI，Peripheral Component Interconnect）</strong>连接到CPU和主存的。不同于与CPU相关的系统总线和内存总线，IO总线和CPU是不相关的。
...
<strong>高速缓存</strong>(cache，"cash")是小而快的，使用高速缓存的过程叫作<strong>缓存(caching)</strong>
存储器层次结构核心思想就是<u><strong>位于k层的更快更小的存储设备是k+1层更大更慢的存储设备的缓存</strong></u>
第k+1层的存储器被划分为连续的数据对象<strong>组块(chunck)</strong>，称为<strong>块(block)</strong>，它的地址唯一确定，大小往往固定(也可以是可变的)
![image-20260719091556879](https://raw.githubusercontent.com/jjh11737/jjh-blog-images/master/imgs/image-20260719091556879.png)
而第k层存储器被划分为较少的块的集合，数据总是以<strong>块</strong>为<strong>传送单元(transfer unit)</strong>在k和k+1层进行传输的
...
写比读更复杂，如果想要写一个已经缓存了的字w(*write hit*)，在高速缓存更新了它的w的副本之后，想要更在层次结构更低一级的副本，就得通过<strong>直写(write through)</strong>，也就是立即将w的高速写到紧接着的低一层中（优点是快而且简单，缺点是增加了复杂性），高速缓存必须为每一个高速缓存行维护一个额外的<strong>脏位(dirty bit)</strong>表明这个高速缓存块是否被修改过
...
在Linux系统，静态库以一种称为<strong>存档(archive)</strong>的特殊文件格式存放在磁盘，它是一组连接起来的可重定位目标文件的集合，有一个头部描述每个成员目标文件的大小和位置，由后缀`.a`标识
...
显然ELF文件设计本身就适合被加载到内存，连续的块chunk映射到连续的内存段，这由<strong>程序头部表(program header table)</strong>描述：
...
我们发明**共享库**主要的目的就是想要<u>多个运行时的进程共享内存中相同库的代码</u>，以节约内存。
而想要实现这一点：
...
因为无论我们加载到内存何处，<strong><u>数据段相对于代码段的偏移总是不变的</u></strong>，因此指令和变量之间的<u>偏移都是一个运行时常量</u>。
因此利用这一点，想要生成PIC的编译器就会在数据段开始的地方创建<strong><u>全局偏移量表(GOT - Global Offset Table)</u></strong>，每个被目标模块引用的全局数据目标都会有一个<u>8字节的地址条目</u>。
...
调用一个共享库定义的函数，**编译器没法预测它的运行时地址**（因为共享库运行时可以加载到任意位置）。
一般的方法是为该引用生成一条重定位记录，动态链接器在程序加载时对它解析，但这就不是PIC了，因为这需要链接器修改调用这个模块的代码片段。（但我们希望一次加载对所有的都可用）
...
符号解析关注3个东西：**未解析的符号集合 / 待解析的目标文件 / 已解析的目标文件**

重定位依赖于重定位条目，它提供节/符号的位置/偏移/地址解析方式

到此为止就可以生成一个可执行的目标文件了，它相比于可重定位会少了`.rel`之类的条目，多了个程序入口也就是`.init`

但是显然这不满足我们的需求，因为我们希望库是可以复用的，而不是每个文件都要复制一遍，能不能只在内存加载一次，之后复用？因此我们提出了动态共享库

那么动态共享库需要什么特征？它必须是位置无关PIC的，不能是写死的地址。而为了实现PIC，就希望
...
在运行时，处理器检测到发生一个事件，并且确定了相应的异常号，随后处理器触发异常，执行间接过程调用，通过异常表的表目$k$转到相应的处理程序。异常号是异常表中的索引，异常表的起始地址放在<strong>异常表基址寄存器(exception table base register)</strong>当中
...
进程的经典定义是<strong><u>一个执行中程序的实例</u></strong>。
系统中的<u>每个程序都运行在某个进程的<strong>上下文(context)</strong></u>中，<strong><u>上下文是由程序正确运行所需要的状态组成的</u></strong>。
...
进程轮流使用处理器，每个进程执行它的流的一部分，然后被<strong>抢占(preempted)</strong>暂时挂起，再移交到其他进程。

#### 8.2.2 并发流

<strong>并发流</strong>指的是<strong><u>一个逻辑流的执行在时间上和另一个流重叠</u></strong>，它们就是并发地在运行。
而<strong><u>一个进程和其他进程轮流运行</u></strong>则称为<strong>多任务</strong>。一个进程执行它的控制流的一部分的每个时间段叫时间片，那么多任务也称时间分片(time slicing)
...
> **别把它和用户栈搞混了**
>
> 用户栈是“**程序执行过程中用于组织调用过程的数据结构**”，而上下文是“**让一个执行流能暂停后从原地继续所必须保存的机器状态**”，用户栈是管理调用链的，是用户态的东西，上下文则是和逻辑控制流切换有关的，属于内核态的东西
...
fork函数虽然被调用1次，却会返回2次：一次在<strong><u>父进程返回子进程PID</u></strong>，一次在<u>子进程返回0</u>（这是为了<strong><u>方便判断是在子进程还是父进程，因为进程PID非零</u></strong>）
...
默认情况下(options = 0)，`waitpid`挂起调用进程的执行，<u>直到它的等待集合中的一个子进程终止</u>。此时<strong><u>直接返回其PID并且其被回收，删除所有痕迹</u></strong>
...
而main开始执行时，<strong>用户栈</strong>如下：（从栈底看）
...
> 应该区分一下fork和execve：
>
> 程序是目标文件，存在磁盘上或作为段存在于地址空间，运行在某个进程的上下文中；而进程则是程序中一个具体的实例，<strong>进程 = 执行流(线程/上下文)+内核资源+地址空间</strong>；
>
> fork干的事情是在新的子进程里面运行完全相同的程序，<strong>子进程只是父进程的一个复制品</strong>（唯一能区分的就是PID了）
>
> 而execve在当前进程的上下文中加载并运行一个新的程序，它会覆盖当前进程的<strong>用户态地址空间（内核态不涉及！！）</strong>整个重新初始化一下，但是还是原来那个进程，原来那个PID。（也就是说它用一个新的程序来替代了旧的程序，但是这个进程本身不变，瓶子不变但装了新的东西）
>
> 但是这两个无论如何<strong>都会保留原先已经打开的文件描述符fd</strong>，因为它是保存在内核空间里面的
...
每种信号都对应某种系统事件，低层次的硬件异常由内核异常处理程序处理，一般来说对用户进程不可见。而<strong><u>信号提供的机制可以通知用户进程发生这些异常</u></strong>。
...
Unix系统提供了大量的向进程发送信号的机制，所有机制都基于<strong>进程组(process group)</strong>这个概念。
...
`setpid`函数<strong><u>把进程`pid`的进程组改为`pgid`</u></strong>：
...
Unix shell使用<strong>作业(job)</strong>这个抽象的概念标识<u>对一条命令行求值而创建的进程</u>
<u><strong>任何时候最多只有1个前台进程和若干个后台进程</strong></u>
...
- 如果`pid = 0`，那么`kill`会发送信号`sig`给<strong><u>调用进程所在进程组</u></strong>的每个进程(包括调用者自身)
...
- G1. **在处理程序中只调用异步信号安全的函数**
  ...
  （原因：要么是<strong><u>可重入的</u></strong>（也就是它不依赖共享的变量），要么是<strong><u>不能被信号程序打断的</u></strong>），如下表为Linux保证安全的系统级函数：
...
为了解决这些问题，Posix标准定义了一组<strong>`sigaction`函数</strong>，<strong><u>允许用户在设置信号处理时明确指定他们想要的信号处理语义</u></strong>（你可以认为是signal的统一版本）
...
这就是一个称为<strong>竞争(race)</strong>的经典同步错误的示例，因为这里的`addjob`和`deletejob`之间存在竞争，这种错误非常难调试（因为它是不可预测的）
...
非本地跳转的一个重要应用就是<strong><u>允许从一个深层嵌套的函数调用中立即返回，这通常是检测到某个错误时</u></strong>。这样我们就不用费时费力地去解开调用栈而是直接恢复到调用前的现场
...
2. `sigjmp`/`longjmp`都不是异步信号安全的，因为它可以跳转到任意代码。因此<strong><u>我们要在`siglongjmp`可达的代码中调用安全的函数</u></strong>
...
而现代处理器使用虚拟寻址的形式，即CPU生成一个虚拟地址访问主存，其间虚拟地址先被转换为适当的物理地址，将虚拟地址转换为物理地址的任务称为<strong>地址翻译(address translation)</strong>，这同样需要CPU硬件和OS的紧密合作。CPU芯片上叫作<strong>内存管理单元(MMU, Memory Management Unit)</strong>专用硬件，利用存放在主存中的查询表来动态翻译虚拟地址，这个表由OS管理
...
将页表条目中的<strong>物理页号(PPN)</strong>和虚拟地址中的VPO串联起来，就得到相应的物理地址。
注意到因为物理和虚拟页都是P字节的，所以<strong>物理页面偏移(PPO, Physical Page Offset)</strong>和VPO是一致的
...
<u>Linux将一个虚拟内存区域与一个磁盘上的<strong>对象(object)</strong>关联起来，以此初始化这个虚拟内存区域的内容</u>，称为**内存映射(memory mapping)**，虚拟内存区域可以映射到2种类型的对象的一种：

1. Linux文件系统的**普通文件**：

   <strong>一个区域可以映射到一个普通磁盘文件的连续部分（如一个可执行目标文件）</strong>。<strong>文件区(section)</strong>被分成页大小的片，每一片包含一个虚拟页面的初始内容。因为按需进行页面调度，所以这些虚拟页面没有真的放入物理内存，知道我们CPU第一次引用这个页。如果区域本身大于文件区，那么剩余部分就会用0填充

2. **匿名文件：**

   **一个区域也可以映射到一个匿名文件**，它是由OS创建的，包含的全是二进制0。当CPU首次引用这样一个虚拟页面时，内核就会在物理内存里面找到一个合适的牺牲页，如果它被修改过，就把此页面换出，用二进制0覆盖牺牲页面并更新页表。而这个**过程中磁盘和内存没有实质的数据传送**，因此映射到匿名文件区域中的页面也叫作**请求二进制0的页(demand-zero page)**

> 而且有趣的是，主存同样并不会直接给它写零，而是我们维护一个零页，然后对所有被认为是0的页我们全都一开始映射到这一个页上，直到我们需要写入时触发COW再额外分配专门的物理页

无论如何，一旦一个虚拟页面被初始化了，它就在一个内核维护的**swap file**之间换来换去，也称swap space / swap area

#### 9.8.1 依旧共享对象

> 内存映射的概念来源于一个发现：如果虚拟内存系统可以集成到传统的文件系统，那么就能提供一种简单而高效地把程序和数据加载到内存的方法
>
> 进程往往会有相同的代码区域，比如每个运行的shell程序的bash进程显然都会有相同的代码；而且程序很多时候要访问只读运行时库代码的相同副本（如libc的printf），那么如第7章所说，进程完全可以共享对象

一个对象可以被映射到虚拟内存的一个区域，要么作为**共享对象**，要么作为**私有对象**。

- **对于共享对象的改变会映射到其他所有共享的进程**，也**会反映到磁盘上**
- **对一个被映射为私有对象的区域做出的一切改变都是对其他进程不可见的，而且任何写操作都不会反映在磁盘上的对象**

> 假设进程1已经把一个共享对象映射到了其虚拟内存的区域当中，而进程2要将同一个共享对象映射到它的地址空间。
> ![image-20260817152943919](https://raw.githubusercontent.com/jjh11737/jjh-blog-images/master/imgs/image-20260817152943919.png)
>
> 因为每一个对象都有唯一的文件名，内核可以立刻判定进程1已经映射
>
> 了对象，而且可以使得进程2页表条目指向相同的物理页面

私有对象采用一种名为**写时复制(COW，copy-on-write)**的技术被映射至虚拟内存，其开始**生命周期的方式基本上和共享对象是一样的，物理内存中只保存有私有对象的一份物理副本**，对每个映射该私有对象的进程，相应的私有区域的页表条目都是**指向同一个物理副本且都是只读的**。
![image-20260817154931607](https://raw.githubusercontent.com/jjh11737/jjh-blog-images/master/imgs/image-20260817154931607.png)

但是一旦有一个进程尝试写入私有区域，那么写操作就会触发一个保护故障，而故障处理程序发现这一点，就会在物理内存创建这个页面的一个新副本，再更新页表条目指向这个副本，然后给它可写的权限。当故障处理程序返回时，重新执行对应的写操作
...
- 通常对象被加载都是只会先有1个物理副本被多个进程共享
- 对于共享对象，内核会把同样的物理地址给不同的进程映射
- 对私有对象，和共享对象唯一的区别就在于<strong><u>如果多个进程共用这片区域则写时复制</u></strong>

#### 9.8.2 依旧fork

1. 当`fork`函数被进程调用时，kernel会为新的进程创建各种数据结构，分配一个唯一的PID
2. 为了给这个新进程创建虚拟内存，kernel会创建当前进程的各种内核数据结构的原样副本。并且把两个进程每个页面都标记为只读，将两个进程的每个区域结构都标记为私有的，写时复制
3. 当`fork`返回时，二者的虚拟内存恰好完全相同，而一旦二者中的一个进行写操作时，写时复制机制就会创建新页面

#### 9.8.3 依旧execve

当我们进行系统调用`execve("a.out", NULL, NULL)`时，就需要加载并运行这个目标文件：

1. 删除已存在的用户区域

   把当前进程虚拟地址里面的所有用户部分的区域结构都删掉

2. 映射私有区域

   把新程序的各个段映射到.text / .data区域，而.bss区域请求二进制0，则是映射到匿名文件

3. 映射共享区域

   如果a.out链接了共享对象/目标，比如libc.so，那么这些对象都是动态链接到这个程序的，通过内存映射到用户虚拟地址空间中的共享区域中

4. 设置程序计数器PC

   最后一件事就是把当前进程上下文的PC设置为代码区的入口点

![image-20260817160933610](https://raw.githubusercontent.com/jjh11737/jjh-blog-images/master/imgs/image-20260817160933610.png)



#### 9.8.4 使用mmap函数的用户级内存映射

Linux进程可以用`mmap`来创建新的虚拟内存区域，把对象映射到这些区域当中。
```c
#include <unistd.h>
#include <sys/mman.h>
void *mmap(void *start, size_t length, int prot, int flags, int fd, off_t offset);
// 返回：成功返回指向映射区域的指针，出错返回MAP_FAILED
```

`mmap`：请求内核在当前进程的虚拟地址空间中建立一段映射，并规定这段虚拟地址区域的来源和访问语义

参数：

- 最好是从`start`开始的一个区域，`start`地址仅仅是一个暗示，通常被定义为NULL

- 把文件描述符`fd`指定的对象的一个连续的片(chunk)映射到这个新的区域，可以为-1表示映射没有对应文件

- 这个连续的对象片大小为`length`字节，

- 从距文件开始处偏移量`offset`字节的地方开始。

- 参数`prot`表示这个新映射的虚拟内存区域的访问权限位

> fd是open返回的，而我们`FILE *fp = fopen(...)`的是包含有fd的

而参数`prot`包含描述新映射的虚拟内存区域的访问权限位（即`vm_prot`位）：（显然你可以`|`）

- PROT_EXEC：区域的页面可以作为指令被CPU执行
- PROT_READ：可读
- PROT_WRITE：可写
- PROT_NONE：无法访问
...
#include <unistd.h>
#include <sys/mman.h>
int mumap(void *start, size_t length);
// 返回：成功0,错误-1
```



> 编写一个mmapcopy.c，用mmap把任意大小磁盘文件复制到stdout，命令行传递输入文件：
> ```c
> #include <unistd.h>
> #include <sys/mman.h>
> #include <stdlib.h>
...
>     exit(1);
>   }
>   fstat(fd, &status);		// 懒得检查了
>   if(status.st_size == 0){
>     close(fd);
>     return 0;
>   }
>   char *sp = NULL;
>   if((sp = mmap(NULL, status.st_size, PROT_READ, MAP_PRIVATE, fd, 0)) == MAP_FAILED){
>     perror("mmap error");
>     exit(1);
>   }
>   write(1, sp, status.st_size);		// 懒得检查了
>   munmap((void *)sp, status.st_size);
>   exit(0);
> }
> ```
...
虽然用`mmap`和`munmap`可以创建和删除虚拟内存区域，但是显然还是<strong>动态内存分配器(dynamic memory allocator)</strong>更方便，也更好移植。堆(heap)

它维护一个进程的虚拟内存区域，也就是所谓**堆(heap)**。
假设堆是一个demand-zero的区域，开始于`.bss`区域之后，向上增长。

<u>**每个进程，内核都会维护一个`brk`指向堆顶**</u>

**分配器将堆视为一组不同大小块(block)的集合来维护**。每个块就是一个连续的虚拟内存片chunk，已分配/空闲：

- **已分配的块显式地保留以供应用程序使用，直到被释放**（要么是应用程序显式地执行，要么是内存分配器自身隐式执行）
- **空闲的块可用来分配，保持空闲直到显式地被应用所分配**

分配器有2种基本风格，**都要求应用显式地分配块，不同支持在于由谁来负责释放**：

- **显式分配器(explicit allocator)**：要求应用显式地释放任何已分配的块。

  > 最经典的就是c的`malloc`程序包，用`malloc`+`free`管理
  > 而C++则是用`new` / `delete`操作符，与前二者相当

- **隐式分配器(implicit allocator)**：要求分配器检测一个已分配的块何时不再被程序使用，释放这个块。也成**垃圾收集器(GC, garbage collector)**，而自动释放未使用的已分配块的过程就是**垃圾收集(garbage collection)**

  > 如Java / Lisp...

内存分配是一个普遍的概念，可以出现在各种上下文中
...
// 成功返回已分配块的虚拟内存地址，失败返回NULL并设置errno
```

`malloc`并不初始化它返回的内存，你应该用`calloc`，他是`malloc`的一个瘦包装函数。而如果想改变已分配块的大小则是`realloc`

动态内存分配器如`malloc`可以通过使用`mmap` / `munmap`函数显示地分配 / 释放堆内存，或者用`sbrk`函数：
```c
#include <unistd.h>
void *sbrk(intptr_t incr);
// 成功返回旧的brk,错误返回-1，设置errno=ENOMEM
```

`sbrk`通过将内核`brk`指针增加`incr`来调节堆的大小。
...
`free`就是通知用户态动态内存分配器：“这块内存不需要了“

> 实际上malloc不会只返回裸数据，还会在用户数据附近维护元数据
>
> 而`free`其实是改变了对应的元数据而非真的初始化了这部分内容。所以你会发现：
> ```c
> free(p);
> printf("%s", p);
> ```
>
> 很多时候其实还是可以正常输出的，它不会修改页表

问题在于它什么都不返回，如果我们给的指针不是指向一个已分配块的起始位置，它的行为甚至是未定义的。。。换句话说你根本不知道有没有错误！它本身就强调一点那就是：**<u>Only pointers returned by calls to `malloc()`, `realloc()` or `calloc()` can be passed to `free()` (dynamically allocated memory on the heap).</u>**
（不过`free(NULL)`是合法的，代表“什么也不做”）

> 我们用一个简单的例子来解释：
> ![image-20260817183423987](https://raw.githubusercontent.com/jjh11737/jjh-blog-images/master/imgs/image-20260817183423987.png)
> `free`的时候只是简单地把那部分地址标记为：可以用，每次malloc就是从头找到可以用的足够大的连续的块

#### 9.9.2 为什么要使用动态内存分配

**最重要的原因：程序往往直到实际运行才可能知道数据结构的大小**

> 因为硬编码一个上限往往是很蠢的行为，它和机器上可用的虚拟内存没有关联，而一旦程序使用者想要使用更大的文件，只能修改这个硬编码重新编译
...
- **处理任意请求序列**
  ...
  唯一约束条件就是：每个释放请求必须对应于一个当前已分配的块1不能假设释放和请求的顺序。
  也因此

- **立即响应请求**

  必须立即响应分配请求，不允许为了提高性能重新排列或缓冲请求

- **只使用堆**

  为了保证分配器可扩展，任何非标量数据结构都必须保存在堆里面
...
那么很显然，在这些限制条件下，分配器需要实现**两个相互冲突的目标**：

- **最大吞吐率**：

  假设我们有n个分配/释放请求序列：$R_0,R_1,R_2,...R_{n-1}$，我们希望分配器吞吐率(单位时间完成请求数)最大化。换句话说目标往往就是让释放和请求的平均时间最小化。那么开发一个具有合理性能的分配器并不困难，也就是说分配请求的最差运行时间与空闲块数量线性关系，释放请求的运行时间则是常数级别

- **最大化内存利用率**：

  一个进程的虚拟空间上限取决于磁盘swap file的大小，加上进程共用，必须高效地使用。
  而想要描述一个分配器使用堆的效率如何，一种有效的标准是**峰值利用率(peak utilization)**：每次请求得到的有效载荷(payload)是p字节，而完成请求$R_k$时，聚集有效载荷(aggregate payload)便是$P_k=\Sigma p_k$ ，而$H_k$表示堆当前的大小，那么前$k+1$个请求的峰值利用率，就是：
  $$
  U_k = \frac {max_{i\le k}P_i}{H_k}
  $$
  我们的目的就是希望整个$U_{n-1}$尽可能大，换句话说就是尽可能多的部分是有效载荷。

显然他们是相互牵制的，想要尽可能高的内存利用率就得花时间遍历，浪费时间，因此需要平衡



#### 9.9.4 碎片fragmentation

堆利用率低主要原因是**碎片现象**：**虽然有未使用的内存但是它们无法用于满足分配的请求**。有两种形式：

- **内部碎片：**

  **一个已分配块比有效载荷大时发生**，换句话说就是把一个小的部分放到一个大的空隙

- **外部碎片：**

  空闲内存本身加起来足以满足一个分配请求，但是**没有一个单独的空闲块足够大来处理这个请求**，此时就必须向内核再次申请额外的虚拟内存

显然**外部碎片比内部碎片更加难以量化/不清晰**，因为它不仅取决于过去请求的模式和分配器，还取决于未来请求的模式，不可能预测。因此往往分配器会使用启发式策略来**维持少量的大空闲块而非大量小空闲块**



#### 9.9.5 实现问题

最简单的分配器会把堆组织为一个大的字节数组和指向顶部的指针`p`。

> 一种很极端的情况：每次`malloc`时我们增加`size`到`p`，并返回`p`旧值到调用函数。`free`则仅仅返回调用函数。这样吞吐量自然是最高的，但是我们从来不重复利用任何块，内存利用率最差。

所以分配器想要做好二者平衡，就得考虑：

- <strong>空闲块组织：</strong>我们如何记录空闲块
- <strong>放置</strong>：如何选择一个合适的空闲块来放置一个新分配的块
- <strong>分割</strong>：在将一个新分配的块放置到某个空闲块之后，如何处理这个空闲块的剩余部分
- <strong>合并</strong>：如何处理一个刚刚被释放的块



#### 9.9.6 隐式空闲链表

**任何实际的分配器都需要一些数据结构，允许它来区别块边界**
多数分配器的方法是将这些信息嵌入块本身，一个简单的方法如下：
![image-20260820163200432](https://raw.githubusercontent.com/jjh11737/jjh-blog-images/master/imgs/image-20260820163200432.png)
这种情况下，一个块是由**一个字大小的头部header** + **有效载荷payload** + **一些额外的填充** 组成的。

头部编码了这个块的大小，以及这个块是已分配还是空闲的。

> 比如我们如果加入一个双字的对齐约束条件，那么块的大小就是8的倍数，这里需要的就是29个高位来表述块的大小，释放剩下的3位来编码其他信息，我们用其中的最低位（已分配位）指明这个块是已分配还是空闲的。
> 比如我们假设有关一个已分配的块，大小24=0x18字节，那么其头部是：`0x00000018 | 0x1 = 0x00000019`

头部后面就是应用调用`malloc`时请求的有效载荷，有效载荷后面是一片不使用的填充块（任意大小），原因有多种：分配器策略/外部碎片/对齐要求...

假设块格式如下：
![image-20260820175303833](https://raw.githubusercontent.com/jjh11737/jjh-blog-images/master/imgs/image-20260820175303833.png)
这种结构就是所谓**隐式空闲链表**，因为**空闲块是通过头部中的大小字段隐含地连接着的**（也就是通过大小就能知道下一个头的地方)，分配器只需要遍历堆中所有块从而间接地遍历整个空闲块的集合。
注意我们需要某种特殊标记的结束块，本示例中就是一个<u>已分配但是大小为0</u>的**终止头部(terminating header)**

> 这里可能有点不好懂？
>
> 

这种设计优点显然是极其简单，缺点在于开销太大（你得一个个遍历过去），复杂度是O(n)级别

而这里比较重要的一点是我们需要意识到**系统对齐要求**和**分配器对块格式的选择**会对分配器上的**最小块大小有强制的要求**（比如我们这里就是双字对齐的）

#### 9.9.7 放置已分配的块

当一个应用请求一个k字节的块时，分配器搜索空闲链表，查找到一个足够大的可以放置所请求的块的空闲块。分配器执行种搜索的方式由**放置策略(placement policy)**，常见的策略是**首次匹配(first fit)** / **下次匹配(next fit)** / **最佳匹配(best fit)**：

- 首次适配：从头开始搜索，选择第一个合适的空闲块

  ​	缺点：会在起始位置制作过多碎片

- 下一次适配：从上一次查询结束的位置开始查询，机制不变

  ​	虽然从直觉上来说似乎是更好，但是根据研究似乎这样的内存利用率反而比首次适配低得多
...
一旦分配器找到一个匹配的空闲块，它就必须做另一个策略决定，那就是**分配多少**。最简单的当然是用整个空闲块，但是碎片会很多。更多时候我们还是会分割为两部分留下空闲块

#### 9.9.9 获取额外的堆内存

如果分配器找不到合适的空闲块，一个选择是尝试把已有的在物理上相邻的空闲块合并来创造更大的空闲块来满足需求(即**coalescing**)，而如果这尚且不能满足，那么就需要通过调用`sbrk`函数向内核请求更多的堆内存。之后分配器把额外的内存转化成一个大的空闲块，将它插入空闲链表后将被请求的块放置在这个新的空闲块中

#### 9.9.10 合并空闲块

当分配器释放一个已分配块时，可能有其他空闲块预期相邻，那么邻接的空闲块会引起**假碎片(fault fragmentation)**：<u>可用的大的空闲块被切割为小的难以使用的空闲块</u>

为了解决这个问题，我们就需要做**合并(coalescing)**。
何时执行coalescing？可以选择immediate coalescing立即合并，每次一个块被释放时就合并所有的相邻块，也可deferred coalescing延迟合并（比如推迟到某个分配请求失败时）

立即合并虽然简单，但是问题是有时会产生抖动，比如反复地合并和分割，为了保证速度，显然还是后者更合适

#### 9.9.11 带边界标记的合并

具体实现：

我们称待释放的块为当前块，合并后面的块显然我们只需要通过当前块头指向的下一个块的头部，判断是否空闲即可，如果是则把其大小简单地加到当前的块上，只需要O(1)的复杂度

但是如果要**合并前面的块**，在目前的链表结构里面只能扫描整个链表，记住前面块的位置，因此Kruth提出添加一个**脚部footer**，即所谓**边界标记(bound tag)**。
这个方法当然很安全简单，但是会要求每个块都要有头部和脚部，一旦程序操作多个小块时就会有显著的内存开销（因为header / footer占比太高了）



#### 9.9.12 实现一个简单的分配器

我们基于隐式空闲链表，使用立即边界合并方式，从头到尾描述一个简单分配器的实现，最大的块是$2^{32}Byte=4GB$的，代码是64位干净的

##### 1. 通用分配器设计

使用`memlib.c`提供的一个内存系统模型，让我们在不干涉已存在的系统malloc包情况下运行分配器。

- `mem_init`：将对堆可用的虚拟内存转化为一个大的 / 双字对齐的字节数组
- 在`mem_brk`(高)和`mem_heap`(低)之间是已分配的虚拟内存
- 通过调用`mem_sbrk`来请求更多内存
  （不过显然并没有涉及系统调用，原本的`sbrk`是一个系统调用借口，我们这里简化了，不包含收缩堆的请求）

> 分配器被放在一个源文件`mm.c`里面，可以把它链接到应用中，它提供3个函数到应用程序：
> ```c
...
输入输出是在主存和外部设备之间复制数据的过程，输入操作：IO设备$\to$主存，输出：主存$\to$IO设备，所有语言的运行时系统都提供执行IO的较高级别工具，比如ANSI C提供的`printf` / `scanf`带缓冲区的IO函数，C++里面定义了`<<` / `>>`。而Unix系统本身也提供了内核的Unix I/O，当然多数时间高级别的IO函数就能很好地工作，但是还是需要了解Unix IO:
...
一个Linux文件就是m个字节的序列：$\{B_0, B_1, B_2, ...\}$，所有的IO设备都被模型化为文件，而所有输入输出都被当作对相应的文件的读写来执行。这就是Unix I/O，保证了所有输入输出以统一一致的方式执行：

- 打开文件：

  一个应用程序通过要求内核打开相应文件来宣告它想要访问一个I/O设备。内核会返回一个小的非负整数称为**描述符**，以此标记后续对此文件的所有操作，内核来记录有关这个被打开文件的所有信息，程序只需要记住描述符

- 任何Linux进程都会有最基本的3个文件描述符：

  0. stdin
  1. stdout
  2. stderr
  
  `<unistd.h>`定义了常量STDIN_FILENO / STDOUT_FILENO / STDERR_FILENO显式代表了描述符的值
